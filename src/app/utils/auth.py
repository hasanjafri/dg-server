...
def generate_api_key():
    random_bytes = hashlib.sha256(str(random.getrandbits(256)).encode('ascii')).digest()
    random_digram = random.choice(['CE', 'Da', 'mA','ni', 'SA', 'St', 'wI']).encode('ascii')
    api_key = base64.b64encode(random_bytes, random_digram).decode('ascii')
